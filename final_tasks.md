...
1. Algebra depth (exponents, quadratics, inequalities, new classifiers): small diffs to proven code, immediate visible win
2. Calculus judge: high wow-factor per line of code, the differentiate-to-verify trick is genuinely simple
3. Chemistry equation balancing plus functional groups: no new dependencies, no recognition risk, demos well typed
4. Hand-drawn structure recognition: highest risk, start the failure log early, lean on the editable-SMILES correction panel as the demo safety net
5. Frontend refactor and segmentation: do the component split before chemistry mode, not after
6. Naming via OPSIN and stoichiometry: nice-to-have breadth once the above holds
