...
`application/x-latex` response, normalized it through `v1`, and never rendered
or transmitted a PNG.
...
| Normalized exact match | 20/30 (66.67%) |
| Deterministic algebra parse success | 20/30 (66.67%) |
| Mean character error rate | 6.254% |
...
reached 55.56% (`n=18`). These labels describe synthetic generator complexity,
...
production gate, deterministic parsing fails for 10 cases, the corpus is
...
device corpus plus a same-input Gemini control. Real student ink remains blocked
on privacy/legal approval, consent, retention, authentication, and commercial
production terms.
