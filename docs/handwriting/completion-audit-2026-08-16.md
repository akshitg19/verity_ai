...
- PR #73 adds exact corpus-governance binding to decision validation, replay
...
| 5 | Reproducible validation/replay/scoring harness | Complete | Schemas, content-safe scorer, durable ledger, MyScript runner, normalization-version enforcement, offline reprocessor, and exact corpus-governance binding are implemented and tested. Every decision validate/plan/score fails closed without a current governance approval and 300–500 fixtures. The activation validator also requires the actual restricted manifest and governance JSON and cross-checks their hashes/IDs/count/provider/retention/deletion evidence without opening stroke/image inputs. |
| 6 | Approved versioned decision corpus | Mechanism complete; external evidence required | The content-free governance schema binds exact hash/count, sources, providers, retention, restricted storage/access/deletion, consent, two-reviewer, and approval-window evidence. The versioned 300-case synthetic v2 geometry corpus remains single-reviewer and decision-ineligible. No actual approved 300–500-case target-device corpus or matching governance approval exists. |
...
| Data/privacy and corpus owners | Approved restricted store; access list; retention/deletion policy and deletion test; provenance/withdrawal/provider-specific consent; 300–500 frozen cases; two reviewers for ambiguous cases | Issue a content-free governance file matching `corpus-governance.schema.json`, then provide both it and the exact restricted manifest so `validate --decision-run --governance ...` and the rollout validator's `--corpus-manifest`/`--corpus-governance` gates pass. Do not put real ink or fake approvals in Git. |
