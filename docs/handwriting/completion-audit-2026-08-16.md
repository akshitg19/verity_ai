...
**Audited repository/frontend source:** PR #68 merge `3b1ca95`; Vercel
...
- PR #68's Vercel production deployment serves the default-off identity code;
...
| 3 | Measured Gemini baseline | External evidence required | Lifecycle instrumentation and a strict schema-v2 export/readiness workflow exist in the [baseline report](baseline-report-2026-08-14.md), but no target-tablet p50/p95 export exists. |
| 4 | Documented teammate A/B path | Mechanism complete; results external | [Internal A/B comparison](internal-ab-comparison.md) provides frozen tasks, session-scoped anonymous pairing, policy/content validation, 3–5-pair readiness gates, metrics, rollback, and aggregation. Results from 3–5 teammates are not attached. |
...
| 17 | Relevant local and remote checks pass | Complete for merged work | PR #67 passes frontend 410 across 43 files, lint, identity-off/on production builds, API-base/provider-secret scans, a negative shared-secret build guard, and a local sign-in-gate smoke. The access-boundary verifier branch passes 20 focused tests and the full backend at 1324 / 3 expected xfails; its exact source also returns `PASS` against the live revision. Linux, Windows, frontend, and Vercel PR/main checks remain required before merge. |
| 18 | Reviewable PRs merged to current main | Complete for safe deliverables | The default-off identity mechanism is merged by PR #67 and its evidence by PR #68 at `3b1ca95`. Cloud Run now deploys that exact source as fully disabled revision `verity-ai-00022-2vj`; Vercel production is current through PR #68. |
| 19 | Preview/staging smoke tests pass | Partially complete | Vercel production for `3b1ca95` and Cloud Build `37fec1f2-8ed5-43dd-b1aa-d004e32bc760` pass. The enhanced Cloud Run verifier proves 100% traffic to disabled revision `verity-ai-00022-2vj`, false MyScript/shared-access flags, identity mode off, empty OAuth/API-secret/allow-list configuration, pinned secret references, and HTTP 200/200/404/200. No target-device authenticated enabled-provider preview is authorized. |
