...
**Repository baseline:** `origin/main` at `3c859d1734cbe15ccd06ace1082290f51fe38a49`  
...
- The 30-call synthetic MyScript run used 30 of its approved 50-attempt ledger
  cap. Normalization `v2` offline rescoring used zero requests and reports
  26/30 exact match and parse success (86.67%), mean character error 2.9206%,
  and provider latency p50/p95 141/266.15 ms.
...
| 6 | Approved versioned decision corpus | External evidence required | The 30-case math and 10-case chemistry corpora are synthetic and single-reviewer. No approved 300–500-case, two-reviewer, target-device corpus with restricted-store policy exists. |
| 7 | Provider commercial/privacy/security terms verified | External evidence required | [Provider readiness](provider-readiness.md) records official public sources and unanswered questions. Written MyScript student-data, training/retention, FERPA/COPPA, production price, minimum, overage, cancellation, SLA, and support answers are missing. |
| 8 | MyScript category evaluation or evidence-backed no-go | External evidence required | The bounded synthetic smoke proves the REST boundary but is decision-ineligible. No category/device comparison against Gemini exists, so neither adoption nor rejection is justified. |
...
| 10 | Selected fast path passes accuracy/parse gate | External evidence required | Synthetic MyScript reaches 86.67%; no predeclared tolerance or eligible same-input Gemini category baseline exists. |
...
| 12 | One-shot observable/correctable rollback-safe hybrid | Mechanism complete; rollout evidence external | PR #47 proves one primary, at most one fallback, no normal-path PNG, timeout abort, cancellation, outage propagation, and no recursion. Visible correction UX and an approved enabled canary remain untested. |
...
| 17 | Relevant local and remote checks pass | Complete for merged work | Latest full local results: backend 1268 passed / 3 expected xfailed; frontend 392 passed across 41 files. Linux, Windows, frontend, and Vercel checks passed through PR #48. |
| 18 | Reviewable PRs merged to current main | Complete for safe deliverables | PRs #31–#48 are merged; latest merge is `3c859d1734cbe15ccd06ace1082290f51fe38a49`. |
...
| MyScript legal/privacy/commercial owner | Written answers for trial/paid data use, training, retention, deletion, region, subprocessors, FERPA/COPPA suitability, attribution/publicity, pricing unit, minimum, overage, cancellation, SLA, and support | Send the prepared vendor questionnaire only after user approval; attach the response and internal go/no-go review. No purchase or contract is authorized here. |
| Product/finance owner | Explicit budget and request cap for a decision corpus; dashboard quota snapshot before and after | Approve a new run identity and cap. The existing authorization ends at 50 attempts and currently has 20 unused slots; it does not authorize 300–500 calls. |
...
Until those owners attach evidence, keep all MyScript flags false, preserve the
20 unused synthetic-smoke slots, and do not send student handwriting to a new
provider.
