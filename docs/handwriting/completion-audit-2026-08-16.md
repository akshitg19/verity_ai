...
| 5 | Reproducible validation/replay/scoring harness | Complete | Schemas, content-safe scorer, durable ledger, MyScript runner, normalization-version enforcement, offline reprocessor, and exact corpus-governance binding are implemented and tested. Every decision validate/plan/score fails closed without a current governance approval and 300–500 fixtures. |
...
| Data/privacy and corpus owners | Approved restricted store; access list; retention/deletion policy and deletion test; provenance/withdrawal/provider-specific consent; 300–500 frozen cases; two reviewers for ambiguous cases | Issue a content-free governance file matching `corpus-governance.schema.json`, then provide the exact restricted manifest so `validate --decision-run --governance ...` passes. Do not put real ink or fake approvals in Git. |
