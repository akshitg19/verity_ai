...
export default function Landing({ theme }) {
  ...
  return (
    <div style={{ minHeight: "100vh", overflowX: "hidden", color: COLORS.text, background: COLORS.background }}>
      <header style={{
        position: "sticky",
        top: 0,
        zIndex: 20,
        ...
        <div style={{ maxWidth: MAX_WIDTH, minHeight: 64, margin: "0 auto", padding: "8px 24px", display: "flex", alignItems: "center", gap: 24 }}>
          <a href="/" aria-label="verity.ai home" style={{ display: "flex" }}><Logo size={34} showWordmark /></a>
      ...
      <Section style={{ paddingBottom: 72 }}>
        <figure style={{ margin: 0 }}>
          <div style={{ overflow: "hidden", border: `1px solid ${COLORS.border}`, borderRadius: RADIUS.xl, background: COLORS.surface, boxShadow: SHADOW.float }}>
            <img
      ...
      <Section style={{ paddingTop: 58, paddingBottom: 58, background: COLORS.surface, borderTop: `1px solid ${COLORS.border}`, borderBottom: `1px solid ${COLORS.border}` }}>
        <div style={{ display: "grid", gridTemplateColumns: grid, gap: 30 }}>
          {BENEFITS.map(([title, body]) => <div key={title}>
      ...
      <Section id="how" style={{ paddingTop: 70, paddingBottom: 70 }}>
        <SectionHeading>Three deliberate steps</SectionHeading>
        <div style={{ marginTop: 32, display: "grid", gridTemplateColumns: grid, gap: 18 }}>
          {STEPS.map(([number, copy]) => <div key={number} style={{ minHeight: 150, padding: 22, boxSizing: "border-box", background: COLORS.surface, border: `1px solid ${COLORS.border}`, borderRadius: RADIUS.lg }}>
      ...
      <Section id="coverage" style={{ paddingTop: 64, paddingBottom: 64, background: COLORS.surface, borderTop: `1px solid ${COLORS.border}`, borderBottom: `1px solid ${COLORS.border}` }}>
        <SectionHeading>Choose a workspace</SectionHeading>
        <div style={{ marginTop: 30, display: "grid", gridTemplateColumns: grid, gap: 22 }}>
          ...
          ].map(([id, title, copy]) => <article key={id} style={{ padding: 28, display: "flex", flexDirection: "column", background: COLORS.background, border: `1px solid ${COLORS.border}`, borderRadius: RADIUS.lg }}>
      ...
      <Section style={{ paddingBottom: 72 }}>
        <div style={{ padding: "42px 28px", color: "#fff", textAlign: "center", background: "linear-gradient(135deg, var(--v-primary), var(--v-chem))", borderRadius: RADIUS.xl }}>
